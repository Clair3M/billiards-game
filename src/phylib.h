...
#define PHYLIB_DRAG (200.0) // mm/s^2
...
// short sides of the table
typedef struct {
    double y;
} phylib_hcushion;

// long sides of the table
typedef struct {
    double x;
} phylib_vcushion;
...
// constructors
phylib_coord phylib_new_coord( double x,
                               double y );
...
// utility functions
void phylib_copy_object( phylib_object **dest,
                         phylib_object **src );

phylib_table *phylib_copy_table( phylib_table *table );

void phylib_add_object( phylib_table *table,
                        phylib_object *object );

void phylib_free_table( phylib_table *table );

phylib_coord phylib_sub( phylib_coord c1,
                         phylib_coord c2 );
                    
double phylib_length( phylib_coord c );

double phylib_dot_product( phylib_coord a, 
                           phylib_coord b );

double phylib_distance( phylib_object *obj1,
                        phylib_object *obj2 );

// physics functions
void phylib_roll( phylib_object *new,
                  phylib_object *old,
                  double time );

unsigned char phylib_stopped( phylib_object *object );

void phylib_bounce( phylib_object **a,
                    phylib_object **b );
